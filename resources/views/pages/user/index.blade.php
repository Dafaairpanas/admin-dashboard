            ...
            <div class="card">
                <div class="card-header">
                    <div class="row align-items-center">
                        ...
                        <div class="col-auto">
                            @if(\App\Helper::hasPermission('USERS', 'create'))
                    ...
                    <form action="{{ route('USERS.read') }}" method="GET" class="mt-3">
                                ...
                                @forelse($data as $index => $user)
                                    <tr>
                                        ...
                                        <td>
                                            @if($user['role_name'])
                                                <span class="badge rounded text-white"
                                                    style="background-color: {{ $user['role_badge'] ?? '#6c757d' }}">
                                                    {{ $user['role_name'] }}
                                                </span>
                                            @else
                                        ...
                                        <td class="text-end">
                                            ...
                                            @if(\App\Helper::hasPermission('USERS', 'delete'))
                            ...
                            <ul class="pagination justify-content-center">
                                @if($attributes['page'] > 1)
                                    <li class="page-item"><a class="page-link"
                                            href="?page={{ $attributes['page'] - 1 }}&search_value={{ $params->search_value }}">Previous</a>
                                    </li>
                                @endif

                                {{-- Simple Logic for Pagination Display --}}
                                <li class="page-item disabled"><span class="page-link">Page {{ $attributes['page'] }} of
                                        {{ $attributes['lastPage'] }}</span></li>

                                @if($attributes['page'] < $attributes['lastPage'])
                                    <li class="page-item"><a class="page-link"
                                            href="?page={{ $attributes['page'] + 1 }}&search_value={{ $params->search_value }}">Next</a>
                                    </li>
                                @endif
                            </ul>
                        </nav>
...
@section('script-bottom')
    <script>
        ...
        editModal.addEventListener('show.bs.modal', function (event) {
            ...
            const roleId = button.getAttribute('data-role_id');

            const form = document.getElementById('editUserForm');
            form.action = '/manage/users/' + id + '/update';
